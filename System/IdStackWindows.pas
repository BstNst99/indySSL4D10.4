...
{$IFDEF NEXTGEN} // !!! HACK ADDED BY EMBT
  ...
  LStr: TPtrWrapper;
begin
  LStr := Marshal.AllocMem(SIZE_HOSTNAME);
  gethostname(MarshaledAString(LStr.ToPointer), SIZE_HOSTNAME);
  ...
  //to string
  Result := TMarshal.ReadStringAsAnsiUpTo(CP_ACP, LStr, SIZE_HOSTNAME);
{$ELSE}
  LStr: AnsiString;
begin
...
begin
  ps := getservbyname(
    MarshaledAString({$IFDEF STRING_IS_UNICODE}Marshal.AllocStringAsAnsi(AServiceName){$ELSE}AServiceName{$ENDIF}),
    nil);
{$ELSE}
  {$IFDEF STRING_IS_UNICODE}
  LTemp: AnsiString;
    ...
    try
      ...
      repeat
        case LAddrInfo^.ai_addr^.sa_family of
          Id_AF_INET: begin
            TIdStackLocalAddressIPv4.Create(AAddresses, TranslateTInAddrToString(PSockAddrIn(LAddrInfo^.ai_addr)^.sin_addr, Id_IPv4), '');                
          end;
          Id_AF_INET6: begin
